 ...
 diagnostics_relay_error_t diagnostics_relay_query_mobilegestalt(diagnostics_relay_client_t client, plist_t keys, plist_t* result)
{
 ...
 diagnostics_relay_error_t diagnostics_relay_query_ioregistry_entry(diagnostics_relay_client_t client, const char* name, const char* clas, plist_t* result)
{
	if (!client || (name == NULL && clas == NULL) || result == NULL)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
	...
	if (clas)
		plist_dict_set_item(dict,"EntryClass", plist_new_string(clas));
