...
static void node_to_xml(node_t* node, void *xml_struct)
{
    ...
    case PLIST_UINT:
        ...
        if (node_data->length == 16) {
			(void)_snprintf(val, 64, "%"PRIu64, node_data->intval);
	} else {
			(void)_snprintf(val, 64, "%"PRIi64, node_data->intval);
	}
        break;
...
static void xml_to_node(xmlNodePtr xml_node, plist_t * plist_node)
{
    ...
    for (node = xml_node->children; node; node = node->next)
    {
        ...
        if (!xmlStrcmp(node->name, XPLIST_INT))
        {
			...
			if (is_negative || (data->intval <= std::numeric_limits<int64_t>::max())) {
        ...
        if (!xmlStrcmp(node->name, XPLIST_DATE))
        {
			...
			data->length = sizeof(data->timeval);
            xmlFree(strval);
            continue;
        }
